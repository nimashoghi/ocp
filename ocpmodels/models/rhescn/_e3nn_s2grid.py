...
def _quadrature_weights(b, dtype=None, device=None):
    ...
    w = torch.tensor(
        [
            (
                (2.0 / b)
                * torch.sin(math.pi * (2.0 * j + 1.0) / (4.0 * b))
                * (
                    (1.0 / (2 * k + 1))
                    * torch.sin((2 * j + 1) * (2 * k + 1) * math.pi / (4.0 * b))
                ).sum()
            )
...
def spherical_harmonics_s2_grid(lmax, res_beta, res_alpha, dtype=None, device=None):
    ...
    shb = o3.Legendre(list(range(lmax + 1)))(
        betas.cos(), betas.sin().abs()
    )  # [b, l * m]
...
def _complete_lmax_res(lmax, res_beta, res_alpha):
    ...
    assert res_beta % 2 == 0
    # assert lmax + 1 <= res_beta // 2 # <-- RH eSCN: This line is commented to allow for res_beta < 2 * (lmax + 1)
...
def _expand_matrix(ls, like=None, dtype=None, device=None):
    ...
    if like is None:
        ...
        m = like.new_zeros(
            (len(ls), 2 * lmax + 1, sum(2 * l + 1 for l in ls)),
            dtype=dtype,
            device=device,
        )
    i = 0
    for j, l in enumerate(ls):
        m[j, lmax - l : lmax + l + 1, i : i + 2 * l + 1] = torch.eye(
            2 * l + 1, dtype=dtype, device=device
        )
        i += 2 * l + 1
    return m
...
def irfft(x, res):
    ...
    *size, sm = x.shape
    ...
    l = res // 2
    x = torch.complex(
        torch.cat([x[:, l : l + 1], x[:, l + 1 :].div(math.sqrt(2))], dim=1),
        torch.cat(
            [
...
class FromS2Grid(torch.nn.Module):
    ...
    def _make_tracing_inputs(self, n: int):
        return [
            {"forward": (torch.randn(self.res_beta, self.res_alpha),)} for _ in range(n)
        ]
