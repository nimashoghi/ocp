...
def compute_neighbors(
    ...
    num_neighbors = segment_coo(ones, edge_index[1], dim_size=natoms.sum())

    # Get number of neighbors per image
    image_indptr = torch.zeros(
        natoms.shape[0] + 1, device=edge_index.device, dtype=torch.long
    )
    image_indptr[1:] = torch.cumsum(natoms, dim=0)
...
def generate_graph(
    ...
    if use_pbc:
        ...
        neighbors = compute_neighbors(natoms, edge_index)
